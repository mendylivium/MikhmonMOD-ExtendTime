...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
	...
	$tawk_l	=	explode("/",$tawk);
	$tawk_postfix	=	$tawk_l[count($tawk_l)-1];
	$tawk_id	=	$tawk_l[count($tawk_l)-2];
	
	$API->comm("/file/print",array("file"=>($has_flash ? 'flash/' : '') . "hotspot/portal"));
	$API->comm("/file/set",array("numbers"=>($has_flash ? 'flash/' : '') . "hotspot/portal.txt","contents"=>"var pisowifi_title = '" . $hotspotname . "', tawk_to = '" . $tawk_id . "/" . $tawk_postfix . "';"));
	
	$hosts = $API->comm("/ip/hotspot/profile/print",array(
		"?dns-name"=>"$dnsname"
	));
	...
	if(!count($API->comm("/ip/hotspot/walled-garden/print",array("?dst-host"=>"*.tawk.to"))) && $tawk != "none") {
		$API->comm("/ip/hotspot/walled-garden/add",array("action"=>"allow","dst-host"=>"*.tawk.to"));
	};
	
	if(!count($API->comm("/ip/hotspot/walled-garden/print",array("?dst-host"=>"*.tawk.link"))) && $tawk != "none") {
		$API->comm("/ip/hotspot/walled-garden/add",array("action"=>"allow","dst-host"=>"*.tawk.link"));
	};
	
	if(!count($API->comm("/ip/hotspot/walled-garden/print",array("?dst-host"=>"*.jsdelivr.net"))) && $tawk != "none") {
		$API->comm("/ip/hotspot/walled-garden/add",array("action"=>"allow","dst-host"=>"*.jsdelivr.net"));
	};
