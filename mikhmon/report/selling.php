		  ...
		  <div class="overflow box-bordered" style="max-height: 70vh">
			...
			if ($prefix != "") {
				...
				for ($i = 0; $i < $TotalReg; $i++) {
					...
					$time_arr = dateTimeConverter(explode("-",$profile)[0]);
						
					echo  ($time_arr['hour'] ? $time_arr['hour'] . ' Hour' . ($time_arr['hour'] > 1 ? 's' : '') : '') . ($time_arr['minutes'] ? $time_arr['minutes'] . ' Minute' . ($time_arr['minutes'] > 1 ? 's' : '') : '');
