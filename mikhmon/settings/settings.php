...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if ($id == "settings" && explode("-",$router)[0] == "new") {
    ...
    fwrite($f, "\n'$'data['".$router."'] = array ('1'=>'".$router."!','".$router."@|@','".$router."#|#','".$router."%','".$router."^','".$router."&Rp','".$router."*10','".$router."(1','".$router.")','".$router."=10','".$router."@!@disable','".$router."!|!none');");
    fclose($f);
    $search = "'$'data";
    $replace = (string)"$data";
  ...
  if (isset($_POST['save'])) {
	...
	$stawk	=	($_POST['tawk']=='' ? 'none' : $_POST['tawk']);
    if ($sreload < 10) {
      $sreload = 10;
    } else {
      $sreload = $sreload;
    ...
    $search = array('1' => "$session!$iphost", "$session@|@$userhost", "$session#|#$passwdhost", "$session%$hotspotname", "$session^$dnsname", "$session&$currency", "$session*$areload", "$session($iface", "$session)$infolp", "$session=$idleto", "'$session'", "$session@!@$livereport","$session!|!$tawk");

    $replace = array('1' => "$sesname!$siphost", "$sesname@|@$suserhost", "$sesname#|#$spasswdhost", "$sesname%$shotspotname", "$sesname^$sdnsname", "$sesname&$scurrency", "$sesname*$sreload", "$sesname($siface", "$sesname)$sinfolp", "$sesname=$sidleto", "'$sesname'", "$sesname@!@$slivereport","$sesname!|!$stawk");
    	   ...
    	   <div class="row">
            ...
            <div class="col-12">
				...
				<table class="table table-sm">
					<tr>
	  					<td class="align-middle">IP of MikroTik </td><td><input class="form-control" type="text" size="15" name="ipmik" title="IP MikroTik / IP Cloud MikroTik" value="<?= $iphost; ?>" required="1"/></td>
					</tr>
					<tr>
...
<div class="col-12">
	<div class="card">
        <div class="card-header">
            <h3 class="card-title">Piso Wifi Data</h3>
        </div>
    <div class="card-body">    
	<table class="table table-sm">
	<tr>
	<td class="align-middle">Piso Wifi Name  </td><td><input class="form-control" type="text" size="15" maxlength="50" name="hotspotname" title="Hotspot Name" value="<?= $hotspotname; ?>" placeholder="e.g Mendys Piso Wifi" required="1"/></td>
	</tr>
	<tr>
	<td class="align-middle">Piso Wifi Link  </td><td><input class="form-control" type="text" size="15" maxlength="500" name="dnsname" title="DNS Name [IP->Hotspot->Server Profiles->DNS Name]" value="<?= $dnsname; ?>" placeholder="e.g men.dy" required="1"/></td>
	</tr>
	<tr>
	<td class="align-middle"><?= $_currency ?>  </td><td><input class="form-control" type="text" size="3" maxlength="4" name="currency" title="currency" value="Php" required="1"/></td>
	</tr>
	<tr>
	<td class="align-middle">Tawk.TO Direct Chat Link  </td><td><br/><small>Chat Support.</small><input class="form-control" name="tawk" title="Tawk.TO Chat Support" value="<?= $tawk ?>" placeholder="(Optional)"/></td>
	</tr>
